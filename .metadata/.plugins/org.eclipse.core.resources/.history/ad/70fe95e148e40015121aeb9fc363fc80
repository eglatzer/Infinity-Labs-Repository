...
public class DateFinder {
	...
	private boolean isGregCalendar(boolean isGregorian_) {
		if(1582 > m_year ||
		  (1582 == m_year &&
		  (10 > m_month.value && 0 < m_month.value ||
		  (10 == m_month.value && 15 > m_day)))) {
			return false;
		}
		
		if(1918 < m_year ||
		  (1918 == m_year &&
		  (1 < m_month.value || 0 == m_month.value))) {
			return true;
		}
	...
	public DateFinder(int year_, Month month_, int day_, boolean isGregorian_) {
		...
		m_month = month_;
		m_day = day_;
		m_isGregorian = isGregCalendar(isGregorian_);
	}
